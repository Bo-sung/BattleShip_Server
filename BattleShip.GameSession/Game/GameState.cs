    ...
    public class GameState
    {
        ...
        public bool SetPlacementDone(int playerIndex)
        {
            ...
            if (PlacementDone[0] && PlacementDone[1])
            {
                ...
                if (_gameMode == (byte)GameModeType.SkillMode)  // StarBattle
                {
                    Phase = GamePhase.SkillSelection;
                    CurrentTurn = new Random().Next(0, 2);  // Set turn even for StarBattle
                }
                else
                {
        ...
        public (byte result, string sunkShipName, bool isGameOver) ProcessAttack(int attackerIndex, byte x, byte y)
        {
            ...
            if (_gameMode == (byte)GameModeType.SkillMode)
            {
        ...
        {
            if (_gameMode != (byte)GameModeType.SkillMode)  // Only for StarBattle
